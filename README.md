...
本脚本同时适配 **重庆大学在线课程平台**（`courses.cqu.edu.cn`）和 **雨课堂**（`yuketang.cn`），解决视频学习中的后台暂停问题，实现无人值守的自动播放体验。

> ⚠️ **注意**：本脚本只能保证**单视频**的强制后台播放，**不支持自动连播**（即当前视频结束后不会自动切换到下一集）。如需播放下一集，需手动切换页面。
...
- 不支持。脚本只保证当前页面视频的强制播放，无法自动跳转到下一集。视频列表中的下一集需要手动点击进入。

> 💡 **小技巧**：可以同时打开多个视频页面（多个标签页），每个页面都会独立强制播放，以此变相实现"批量刷课"。
