...
> ⚠️ **注意**：本脚本只能保证**单视频**的强制后台播放，**不支持自动连播**（即当前视频结束后不会自动切换到下一集）。如需播放下一集，需手动切换页面。**视频中弹出的题目需手动作答**，脚本不会自动处理。
...
- **题目提醒** — 视频中弹出的题目**不自动处理**，需手动作答后继续播放
...
- 需要自己答题。脚本不处理题目弹窗，遇到答题请手动完成后再继续播放。
