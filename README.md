...
> ⚠️ **注意**：本脚本只能保证**单视频**的强制后台播放，**不支持自动连播**（即当前视频结束后不会自动切换到下一集）。如需播放下一集，需手动切换页面。**视频中弹出的题目需手动作答**（弹题时视频不会暂停），脚本不会自动处理。
...
- **题目提醒** — 视频中弹出的题目**不自动处理**，需手动作答。弹题时视频不会暂停，可答完题后继续观看
...
- 不支持。脚本只保证当前页面视频的强制播放，无法自动跳转到下一集。但可以多开标签页并行播放（见上方功能特点）。

**视频中途弹出题目怎么办？**

- 需要自己答题。脚本不处理题目弹窗，但视频不会因弹题而暂停，可以慢慢答完再继续看。
